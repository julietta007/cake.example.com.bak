...
class ArticlesController extends AppController
{
    ...
    public function add()
   {
       if (empty($this->request->session()->check('Auth.User.id'))) {
           $this->Flash->error(__('Your need to be logged into do that.'));
    ...
    public function edit($slug)
    {
        if (empty($this->request->session()->check('Auth.User.id'))) {
            ...
            $this->Flash->eror(__('Unable to update your article.'));
        }
    ...
    public function delete($slug)
    {
        if (empty($this->request->session()->check('Auth.User.id'))) {
            $this->Flash->error(__('Your need to be logged into do that.'));
            return $this->redirect('/');    $this->request->allowMethod(['post', 'delete']);
    }
        $article = $this->Articles->findBySlug($slug)->firstOrFail();
